...
//            " AND ufa.status = 'A' and cgm.status = 'A' UNION SELECT 0))\n" +
    ...
    private static final String GET_PATIENT_REGISTRATIONS_FOR_VISIT = "SELECT pd.mr_no, pd.patient_gender, pd.patient_address,\n" +
            "\tct.city_name, sm.state_name, pd.patient_phone,\n" +
            "\tcm.country_name,\n" +
            "\tpd.user_name,\n" +
//            "\tpd.expected_dob, \n" +
...
//            "\tpd.first_visit_reg_date, \n" +
    ...
    public List<PatientVisit> getPatientRegistrationsVisits(final String visits){
        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("visitIds", visits);
        List<PatientVisit> patientDetails = namedParameterJdbcTemplate.query(GET_PATIENT_REGISTRATIONS_FOR_VISIT, params,
                (rs, rowNum) -> {
                    PatientVisit patientVisit = new PatientVisit(StringUtils.isNotBlank(rs.getString("patient_id")) ? rs.getString("patient_id") : rs.getString("mr_no"));
                    patientVisit.setMrNo(rs.getString("mr_no"));
                    patientVisit.setPatientId(rs.getString("patient_id"));
